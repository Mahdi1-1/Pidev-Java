...
public class FormPredictionController {
    ...
    @FXML private CheckBox diabeteCheckBox;
    ...
    public void initialize() {
        ...
        if (diabeteCheckBox != null) {
            diabeteCheckBox.setDisable(true);
        }
    }
    ...
    public void setPrediction(Prediction prediction) {
        ...
        if (prediction != null) {
            ...
            diabeteCheckBox.setSelected(prediction.isDiabete());
        }
    }
    ...
    private void enregistrerPrediction() {
        try {
            ...
            // Update the UI to reflect the API prediction
            diabeteCheckBox.setSelected(hasDiabetes);

            // Save to the database
            ServicePrediction service = new ServicePrediction();
            if (prediction == null) {
                service.ajouter(tempPrediction);
                showAlert("Succès", "Prédiction ajoutée avec succès !");
            } else {
                service.modifier(tempPrediction);
                showAlert("Succès", "Prédiction modifiée avec succès !");
            }

            // If prediction is 1 (diabetes), send email to doctor and patient
            if (hasDiabetes) {
                /*if (doctorEmail == null) {
                    throw new Exception("L'email du docteur n'est pas défini.");
                }*/
