...
public class ServiceDossierMedical implements IService<DossierMedical> {
    ...
    // Méthode pour filtrer et paginer les dossiers médicaux
    public List<DossierMedical> filterDossiers(String unite, LocalDate date, String searchText, int page, int pageSize) throws SQLException {
        ...
        if (searchText != null && !searchText.isEmpty()) {
            query.append(" AND (CAST(id AS CHAR) LIKE ? OR CAST(utilisateur_id AS CHAR) LIKE ? OR fichier LIKE ? OR unite LIKE ?)");
    ...
    // Méthode pour compter le nombre total de dossiers médicaux (pour la pagination)
